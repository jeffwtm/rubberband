...
exports.signAndNotarize = async (options, logData) => {
    const { sh, password, buildPath, gamemakerPath, appSource, appDest, appNotarized, appleUser, applePassword, providerShortName, signingCertificate, bundleid, localOutputPath, useEntitlements } = options;
    ...
    const notarizeApp = async () => {
        ...
        const errors = notarizationResult.filter(result => result && typeof result == 'string' && result.match(/Error: .*/));

        // const errors = notarizationResult.toString().match(/Error: .*/g);
        if (errors.length) {
            errors.forEach(error => logData(error));
            return;
        }
